...
export async function redirectToCheckout() {
  try {
    ...
    if (!cart.checkoutUrl) {
      // If no checkout URL, try to create a new cart
...
export async function createCartAndSetCookie() {
  try {
    // First try to get existing cart if we have an ID
    const existingCartId = cookies().get('cartId')?.value;
    let cart;
    ...
    // If no existing cart or it's invalid, create a new one
    if (!cart || !cart.checkoutUrl) {
      cart = await createCart();
      if (!cart?.id) {
        throw new Error('Failed to create cart');
      }
      // Type check to ensure cart.id is defined before setting cookie
      if (typeof cart.id === 'string') {
        cookies().set('cartId', cart.id);
      }
    }

    // Verify we have all necessary cart data
    if (!cart.checkoutUrl) {
      cart = await createCart();
      if (!cart?.id || !cart.checkoutUrl) {
        throw new Error('Failed to create cart with checkout URL');
      }
      // Type check to ensure cart.id is defined before setting cookie
      if (typeof cart.id === 'string') {
        cookies().set('cartId', cart.id);
