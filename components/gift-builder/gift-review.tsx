...
export function GiftReview() {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      // Add gift box first
      const boxResult = await addItem({}, state.selectedBox.variantId);
      ...
      for (const product of state.selectedProducts) {
        ...
        if (result === 'Error adding item to cart') {
          throw new Error(`Failed to add ${product.title} to cart`);
        }
      }
...
// // components/gift-builder/gift-review.tsx
// 'use client';
...
//           <div className="flex items-center gap-4">
//             <div className="relative h-24 w-24 overflow-hidden rounded-lg">
//               {state.selectedBox.image ? (
//                 <Image
//                   src={state.selectedBox.image}
//                   alt={state.selectedBox.title}
//                   fill
//                   className="object-cover"
//                 />
