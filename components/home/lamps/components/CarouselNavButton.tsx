...
export const CarouselNavButton = memo(function CarouselNavButton({
  ...
  return (
    <button
      className={`custom-swiper-button-${direction} ${className} group absolute top-1/2 isolate z-20 hidden h-14 w-10
        ...
        ${direction === 'prev' ? 'left-0 -translate-x-full rounded-l-md' : 'right-0 translate-x-full rounded-r-md'}`}
    ...
    >
      ...
      {/* Animated border */}
      <div className={`absolute inset-0 z-30 opacity-0 transition-opacity duration-300
        ${direction === 'prev' ? 
